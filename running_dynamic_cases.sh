...
dynamic_script="generate_dynamic_workload.py"
...
PARENT_FOLDER="dynamic_workload_tuning_both"
...
CASE_NAME="default_with_reloads"
...
"-jar rbe.jar -WINDOW ${MEASURING_WINDOW} -EB rbe.EBTPCW${STARTING_MIX}Factory ${STARTING_EBS} -OUT $FOLDER_NAME/$CASE_NAME.m -RU $RU -MI $MI -RD $RD " \
...
nohup python3 ${dynamic_script} > dynamic_workload.log &
